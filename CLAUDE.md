...
Cloudflared exposes both publicly.

```
ai.htk-hrt.cc     → :8001 proxy.py        → :8002 router (chat)
embed.htk-hrt.cc  → :8003 embed_proxy.py  → :8004 router (embeddings)
```

## Code

- `proxy.py` — chat proxy. Owns the chat router process, generates `models-preset.ini` at startup from `MODELS × CTX_CHOICES`, load/unload via `/models/load` & `/models/unload`, idle-unloads after 10 min. Has chat logging (`logs/chat-<date>.log`) and SSE chunk capture.
